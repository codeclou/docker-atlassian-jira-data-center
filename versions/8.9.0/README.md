...
```bash
sudo su
echo "127.0.0.1  jira-cluster-890-lb" >> /etc/hosts
```
...
**(1) Start a Jira Data Center 8.9.0 Cluster**

```bash
rm -rf /opt/jira-cluster/8.9.0 | true
mkdir -p /opt/jira-cluster/8.9.0
mkdir -p /opt/jira-cluster/8.9.0/jira-home-shared
mkdir -p /opt/jira-cluster/8.9.0/jira-home-node1
mkdir -p /opt/jira-cluster/8.9.0/jira-home-node2
# If on linux fix permissions for volume mounts
# sudo chown 2001:2001 /opt/jira-cluster/8.9.0/jira-*
```
...
- Open a browser to [http://jira-cluster-890-lb:1890/](http://jira-cluster-890-lb:1890/)
...
15ed1263c551        loadbalancer:v2                 "/work-private/docke…"   0.0.0.0:1890->1890/tcp   jira-cluster-890-lb
2994d0d680ad        atlassian/jira-software:8.9.0   "/tini -- /entrypoin…"   8080/tcp                 jira-cluster-890-node1
572fcaf9f669        postgres:9.6                    "docker-entrypoint.s…"   5432/tcp                 jira-cluster-890-db
```
...
Go to **[http://jira-cluster-890-lb:1890/](http://jira-cluster-890-lb:1890/)** and make sure you enabled cookies (sticky session).
...
Set the baseUrl to `http://jira-cluster-890-lb:1890` and continue.
...
```
cp -R /opt/jira-cluster/8.9.0/jira-home-node1/{data,plugins,logos,import,export,caches} /opt/jira-cluster/8.9.0/jira-home-node2
```
...
"https://raw.githubusercontent.com/codeclou/docker-atlassian-jira-\
data-center/master/versions/8.9.0/docker-compose-two-nodes.yml"

docker-compose -f docker-compose-two-nodes.yml up -d
docker-compose -f docker-compose-two-nodes.yml restart jira-cluster-890-lb
```
...
```bash
cd /opt/jira-cluster/8.9.0
mkdir -p /opt/jira-cluster/8.9.0/jira-home-node3
mkdir -p /opt/jira-cluster/8.9.0/jira-home-node4
# If on linux fix permissions for volume mounts
# sudo chown 2001:2001 /opt/jira-cluster/8.9.0/jira-*
```
...
```
cp -R /opt/jira-cluster/8.9.0/jira-home-node1/{data,plugins,logos,import,export,caches} /opt/jira-cluster/8.9.0/jira-home-node3
```
...
"https://raw.githubusercontent.com/codeclou/docker-atlassian-jira-\
data-center/master/versions/8.9.0/docker-compose-three-nodes.yml"
...
"https://raw.githubusercontent.com/codeclou/docker-atlassian-jira-\
data-center/master/versions/8.9.0/docker-compose-four-nodes.yml"
...
```
curl -I -s http://jira-cluster-890-lb:1890 | grep X-ANODEID
```
...
#...
# DOCKER jira-cluster-890 network
