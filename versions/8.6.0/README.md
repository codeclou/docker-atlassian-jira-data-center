...
We follow the [Adding a second Node Guide](https://confluence.atlassian.com/adminjiraserver075/installing-jira-data-center-935390868.html#InstallingJIRADataCenter-5.AddanewJIRAnodetothecluster).

At first copy the Jira home directory files from node1 to node2. (Not all folder might exist. `caches` and `plugins` should.)

```
cp -R /opt/jira-cluster/8.6.0/jira-home-node1/{data,plugins,logos,import,export,caches} /opt/jira-cluster/8.6.0/jira-home-node2
```
...
```
cp -R /opt/jira-cluster/8.6.0/jira-home-node1/{data,plugins,logos,import,export,caches} /opt/jira-cluster/8.6.0/jira-home-node3
```
