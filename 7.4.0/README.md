...
Start an [Atlassian JIRA® Software Data Center](https://de.atlassian.com/enterprise/data-center) version 7.4.0 with Docker for local testing during plugin development.
...
To start, scale, stop and destroy the cluster, the [`manage-jira-cluster-7.4.0.sh`](https://github.com/codeclou/docker-atlassian-jira-data-center/blob/master/7.4.0/manage-jira-cluster-7.4.0.sh) script is provided.
It basically works in the following way:


  * It creates a docker bridge network called `jira-cluster-740`.
  * It creates a shared volume called `jira-shared-home-740`.
  * It creates a docker container called `jira-cluster-740-lb` as the loadbalancer.
    * Binds Port 50611/tcp to the host.
  * It creates a docker container called `jira-cluster-740-db` as the database.
    * Port 5432/tcp are only exposed inside the `jira-cluster-740` network.
  * It creates multiple docker containers called `jira-cluster-740-node{n}` as JIRA Software instances.
    * With `n ∈ {1,2,3,...,100}`.
    ...
    * Port 8080/tcp and the multicast Ports 40001,4446/tcp are only exposed inside the `jira-cluster-740` network.
...
The script is meant to follow the convention over configuration paradigma, therefore there is not much to be configured, except one thing:
  ...
  * It relies on the hostname `jira-cluster-740-lb` pointing to the interface which binds 60740/tcp.
...
```bash
sudo su
echo "127.0.0.1  jira-cluster-740-lb" >> /etc/hosts
```
...
#
curl -so /usr/local/bin/manage-jira-cluster-7.4.0.sh \
"https://raw.githubusercontent.com/codeclou/docker-atlassian-jira-\
data-center/master/7.4.0/manage-jira-cluster-7.4.0.sh"
...
# MAKE EXECUTABLE
#
chmod +x /usr/local/bin/manage-jira-cluster-7.4.0.sh
```
...
**(1) Start a JIRA® Data Center 7.4.0 Cluster**

```bash
manage-jira-cluster-7.4.0.sh --action create --scale 1
```
 ...
 * Open a browser to [http://jira-cluster-740-lb:60740/](http://jira-cluster-740-lb:60740/)
...
```bash
manage-jira-cluster-7.4.0.sh --action info
```

Should show something like:

```
101c71ae0c12    jira-cluster-740-node1     4446/tcp, 8080/tcp, 40001/tcp
e2e9a6b1b757    jira-cluster-740-db        5432/tcp
72f92316309f    jira-cluster-740-lb        0.0.0.0:60740->60740/tcp
```

You can check the logs of each container by calling e.g.:

```bash
docker logs jira-cluster-740-node1
```
...
Go to **[http://jira-cluster-740-lb:60740/](http://jira-cluster-740-lb:60740/)** and make sure you enabled cookies (sticky session).
...
```bash
manage-jira-cluster-7.4.0.sh --action update --scale 3
```
...
```bash
manage-jira-cluster-7.4.0.sh --action restart-node --id 3
```
...
```bash
manage-jira-cluster-7.4.0.sh --action destroy
```
...
#...
# DOCKER jira-cluster-740 network
